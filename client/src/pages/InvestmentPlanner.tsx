    ...
    <div className="space-y-6">
      {dashboard?.goal && dashboard.goalProjection && (
        <section className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          <h2 className="mb-1 text-sm font-medium text-gray-500 dark:text-gray-400">{dashboard.goal.name}</h2>
          <p className="text-sm">
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400">Instruments</h2>
        {instruments.map((inst) => {
          const latest = [...inst.contributions].sort((a, b) => (a.month < b.month ? 1 : -1))[0];
          const f = contribForms[inst.id] ?? { amount: "", runningValue: "" };
          return (
            <div key={inst.id} className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
              ...
              <div className="mt-2 flex gap-2">
                ...
                />
                <input
                  className="w-28 rounded border border-gray-300 dark:border-gray-600 px-2 py-1 text-xs"
      ...
      <section className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
        <h2 className="mb-2 text-sm font-medium text-gray-500 dark:text-gray-400">Add instrument</h2>
